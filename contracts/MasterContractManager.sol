...
contract MasterContractManager is MasterContractFactory, Ownable, EIP712 {
    ...
    function setMasterContractApproval(
        ...
        if (v == 0 && r == 0 && s == 0) {
             ...
             */
            require(user == _msgSender(), "MCM: user must be the sender");
            require(
                masterContractOf[user] == address(0),
                "MCM: user must not be registered"
            );
            require(
                whitelistedMasterContracts[masterContract],
                "MCM: masterC not registered"
            );
        } else {
                        ...
                        keccak256(
                            abi.encode(
                                ...
                                approvalState
                                    ...
                                    : keccak256(
                                        "Revoke access to Midas kingdom V1"
                                    ),
                                user,
                                masterContract,
                                approvalState,
     ...
     *@event LogWhitelistMasterContract
     */
    function whitelistMasterContract(address masterContract, bool approvedState)
        external
        onlyOwner
    {
        require(masterContract != address(0), "MCM: cannot be ZERO Address");
